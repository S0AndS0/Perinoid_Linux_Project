Torrc_service_configs(){
	...
	for _open_service in ${_open_services//,/ }; do
		...
		web)
			...
			echo "HiddenServicePort ${_nginx_http_port} 127.0.0.1:${_tor_web_port}" | tee -a ${_tor_dir}/torrc-open-web
		...
		mirror)
			...
			echo "HiddenServicePort ${_nginx_http_port} 127.0.0.1:${_tor_web_port}" | tee -a ${_tor_dir}/torrc-open-mirror
		...
		ssh)
			...
			echo "HiddenServicePort ${_openssh_port} 127.0.0.1:${_tor_ssh_port}" | tee -a ${_tor_dir}/torrc-open-ssh
